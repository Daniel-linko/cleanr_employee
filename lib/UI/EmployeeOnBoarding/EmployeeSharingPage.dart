...
class EmployeeSharingPage extends StatelessWidget {
  ...
  Widget linkButton(
    ...
    if (employee.employeeInformationModel?.firstNameAttribute?.value != null)
      displayName =
          employee.employeeInformationModel?.firstNameAttribute?.value ??
              "Undefined";
    if (employee.employeeInformationModel?.lastNameAttribute?.value != null)
      displayName = displayName +
          " " +
          (employee.employeeInformationModel?.lastNameAttribute?.value ?? "");

    return FutureBuilder<Uri>(
      future: Sharing.createDynamicLink(employee.userID(), androidPackage,
          iosPackage, iosAppStoreID, linkType, domain, displayName),
